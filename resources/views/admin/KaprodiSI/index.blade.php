...
@section('content')
    <h1 class="fw-bold mb-3" style="text-align: center; font-size: 40px; padding-top:100px; color:rgb(0, 0, 112);">Data Ketua Program Studi Sistem Informasi</h1>
    ...
    <div class="card-header">
        <div class="d-flex align-items-center">
            <a href="{{ route('kaprodi.createKaprodiSI') }}" class="btn btn-primary btn-round ms-auto" style="margin-right: 100px; font-size: 17px;">
                    ...
                    @foreach ($dataKaprodiSI as $data)
                        <tr>
                            ...
                            @if ($data->user->id_roles == 2)
                                <!-- Tombol Edit -->
                                <a href="{{ route('kaprodi.editKaprodiSI', ['id' => $data->user->id]) }}" class="btn btn-warning btn-sm">
                                Edit                                             
                                </a>
                                <!-- Tombol Hapus -->
                                <form action="{{ route('kaprodi.deleteKaprodiSI', ['id' => $data->user->id]) }}" method="POST" style="display:inline;">
                                    @csrf
                                    @method('DELETE')
